...
const COMPILE_CUDA = "nvcc --linker-options=\"-fPIC\" --compiler-options=\"-fPIC\" --shared -g"
...
const PATH_TEND_Z_CALC_KERNEL = joinpath(@__DIR__, "tend_z_calc.knl") 
...
function main(args::Vector{String})
    ...
    @jkernel PATH_REDUCTION_KERNEL reduce_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_X_KERNEL tend_x_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_X_CALC_KERNEL tend_x_calc_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_Z_KERNEL tend_z_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_Z_CALC_KERNEL tend_z_calc_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_APPLY_KERNEL tend_apply_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_APPLY1_KERNEL tend_apply1_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_APPLY2_KERNEL tend_apply2_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_HALO_1RANK_KERNEL halo_1rank_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_HALO_SENDBUF_KERNEL halo_sendbuf_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_HALO_RECVBUF_KERNEL halo_recvbuf_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_HALO_INJECT_KERNEL halo_inject_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_HALO_Z_KERNEL halo_z_kernel mini framework(cuda=COMPILE_CUDA, fortran=COMPILE_FORTRAN) 
...
#function apply1(state_out::OffsetArray{Float64, 3, Array{Float64, 3}},
#                    dt::Float64,
#                    hy_dens_cell::OffsetVector{Float64, Vector{Float64}})
#        @jenterdata mini updateto(state_out, tend)
#        THREADS = ((NX, NZ, NUM_VARS),1)
#         @timeit to "update" @jlaunch tend_apply1_kernel mini input(state_out, tend, hy_dens_cell, dt) output(state_out, tend) 
#        @jexitdata mini updatefrom(state_out)
#end
#function apply2(state_init::OffsetArray{Float64, 3, Array{Float64, 3}},
...
#        THREADS = ((NX, NZ, NUM_VARS),1)
#         @timeit to "update" @jlaunch tend_apply1_kernel mini input(state_init, tend, hy_dens_cell, dt) output(state_out, tend) 
#        @jexitdata mini updatefrom(state_out)
#end
#
...
#    if pointer(parent(state_init)) == pointer(parent(state_out))
#        apply1(state_out, dt, hy_dens_cell)
#
#    else
#        apply2(state_init, state_out, dt, hy_dens_cell)
#
#    end
        ...
        #@jenterdata mini updateto(state_init, tend)
        THREADS = ((NX, NZ, NUM_VARS),1)
         @timeit to "update" @jlaunch tend_apply_kernel mini input(state_init, tend, hy_dens_cell, dt) output(state_out, tend)  cuda(threads=THREADS)
        #@jexitdata mini updatefrom(state_out)


#    #println("PPPPP ", pointer_from_objref(parent(state_init)), " ",  pointer_from_objref(parent(state_out)))
#    #if parent(state_init) == parent(state_out)
#    if pointer_from_objref(parent(state_init)) == pointer_from_objref(parent(state_out))
#        println("EEEE SAME")
#        @jenterdata mini updateto(state_out, tend)
#        THREADS = ((NX, NZ, NUM_VARS),1)
#         @timeit to "update" @jlaunch tend_apply1_kernel mini input(state_out, tend, hy_dens_cell, dt) output(state_out, tend) fortran()
#        @jexitdata mini updatefrom(state_out)
#
#    else
#        println("EEEE DIFF")
#        @jenterdata mini updateto(state_init, tend)
#        THREADS = ((NX, NZ, NUM_VARS),1)
#         @timeit to "update" @jlaunch tend_apply2_kernel mini input(state_init, tend, hy_dens_cell, dt) output(state_out, tend) fortran()
#        @jexitdata mini updatefrom(state_out)
#
#    end
#  
...
#        @jlaunch halo_1rank_kernel mini input(state) output(state) cuda(threads=THREADS)
...
#    end
    ...
    #@jlaunch halo_sendbuf_kernel mini input(state) output(sendbuf_l, sendbuf_r) cuda(threads=THREADS)
    ...
    THREADS = ((HS, NZ, NUM_VARS), 1)
    #@jlaunch halo_recvbuf_kernel mini input(recvbuf_l, recvbuf_r) output(state) cuda(threads=THREADS)
...
#    @jenterdata updateto(state)
    ...
    THREADS = ((NX+1, NZ),1)
    @jlaunch tend_x_kernel mini input(state, dt,hy_dens_cell, hy_dens_theta_cell) output(flux, tend) cuda(threads=THREADS)
#    @jexitdata mini updatefrom(flux)
 
#    @jenterdata updateto(flux)
    THREADS = ((NX, NZ, NUM_VARS),1)
    @jlaunch tend_x_calc_kernel mini input(flux) output(tend) cuda(threads=THREADS)
...
function set_halo_values_z!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    @jlaunch halo_z_kernel mini input(state, hy_dens_cell) output(state,)  cuda(threads=THREADS)
    #@jexitdata mini updatefrom(state)
...
function compute_tendencies_z!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    #@jenterdata updateto(state)
    THREADS = ((NX, NZ+1),1)
    @jlaunch tend_z_kernel mini input(state, dt,hy_dens_int, hy_dens_theta_int, hy_pressure_int) output(flux, tend) cuda(threads=THREADS)
#    @jexitdata mini updatefrom(flux)
 
#    @jenterdata updateto(flux)
    THREADS = ((NX, NZ, NUM_VARS),1)
    @jlaunch tend_z_calc_kernel mini input(state, flux) output(tend) cuda(threads=THREADS)
