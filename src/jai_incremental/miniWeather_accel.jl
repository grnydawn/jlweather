...
function main(args::Vector{String})
    ...
    # device section: CPU or GPU or others, but only ONE
    # two @jaccel if CPU and GPU are used at the same time?
    # @jsystem to support global setting?
    ...
    @jkernel mini tend_x_kernel PATH_TEND_X_KERNEL  framework(hip=COMPILE_HIP)
    ...
    # TODO : update AccelInterfaces to support framework clause to generate shared lib.
...
function compute_tendencies_x!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    @jlaunch mini tend_x_kernel input(state, dt,hy_dens_cell, hy_dens_theta_cell) output(flux, tend) hip("chevron"=>launchconf)
end
