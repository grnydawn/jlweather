...
function main(args::Vector{String})
    ...
    @jkernel PATH_HALO_Z_KERNEL halo_z_kernel  mini framework(hip=COMPILE_HIP)

    @jkernel PATH_TEND_Z_KERNEL tend_z_kernel  mini
    #@jkernel PATH_TEND_Z_KERNEL tend_z_kernel  mini framework(hip=COMPILE_HIP)

    @jkernel PATH_TEND_Z_CALC_KERNEL tend_z_calc_kernel  mini
    #@jkernel PATH_TEND_Z_CALC_KERNEL tend_z_calc_kernel  mini framework(hip=COMPILE_HIP)

    @jkernel PATH_TEND_APPLY_KERNEL tend_apply_kernel  mini
    #@jkernel PATH_TEND_APPLY_KERNEL tend_apply_kernel  mini framework(hip=COMPILE_HIP)
    ...
    elapsedtime = @elapsed while etime < SIM_TIME
        ...
        #@jenterdata updateto(state, statetmp)
...
#    @jexitdata updatefrom(state_init, state_forcing, state_out, tend)
#    println("UUUUUUUUUUUUU", sum(state_init), " : ", sum(tend))
 
#    THREADS = ((NUM_VARS, NZ, NX), 1)
#    @jenterdata updateto(state_init, tend, hy_dens_cell)
...
#    @jexitdata updatefrom(state_out)
   ...
   #@jexitdata updatefrom(state_out) 
    #println("TTTTTTTTTTTT", sum(state_out))
...
function set_halo_values_x!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    if NRANKS == 1
        ...
        println("EEEEEEE : ", state[-1, 1, 1], " : ", state[NX-1, 1, 1])
...
#    THREADS = ((NUM_VARS, NX+2*HS),1)
#    @jenterdata updateto(state, hy_dens_cell)
...
#    THREADS = ((NZ+1, NX),1)
#    @jenterdata updateto(state, hy_dens_int, hy_dens_theta_int, hy_pressure_int)
#    @jlaunch tend_z_kernel mini input(state, dt, hy_dens_int, hy_dens_theta_int, hy_pressure_int) output(flux) hip(threads=THREADS)
#    @jexitdata updatefrom(flux)
...
#    THREADS = ((NUM_VARS, NZ, NX),1)
#    @jenterdata updateto(state, flux, tend)
