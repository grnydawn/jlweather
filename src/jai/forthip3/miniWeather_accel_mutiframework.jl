...
const COMPILE_FOMPTARGET_CRAY = "ftn -shared -fPIC -h omp,noacc"
...
if parsed_args["framework"] == ""
    ...
    const USE_FORTACC = true
    const USE_FORTOMP = true
    const USE_CUDA = true
    const USE_HIP = true

else
    enabled_frameworks = split(parsed_args["framework"], ",")

    const USE_FORTRAN = "fortran" in enabled_frameworks
    const USE_FORTACC = "fortran_openacc" in enabled_frameworks
    const USE_FORTOMP = "fortran_omptarget" in enabled_frameworks
    const USE_CUDA = "cuda" in enabled_frameworks
    const USE_HIP = "hip" in enabled_frameworks
end
...
#    @jkernel PATH_TEND_Z_CALC_KERNEL tend_z_calc_kernel  mini framework(hip=COMPILE_HIP, fortran=COMPILE_FORTRAN)
...
#    @jkernel PATH_HALO_Z_KERNEL halo_z_kernel  mini framework(hip=COMPILE_HIP, fortran=COMPILE_FORTRAN)
    ...
    @jkernel PATH_TEND_X_CALC_KERNEL tend_x_calc_kernel  mini
    @jkernel PATH_TEND_Z_KERNEL tend_z_kernel  mini
    @jkernel PATH_TEND_Z_CALC_KERNEL tend_z_calc_kernel  mini
    ...
    elapsedtime = @elapsed while etime < SIM_TIME
        ...
        @jexitdata mini updatefrom(state, statetmp)
        @jwait mini
...
function semi_discrete_step!(state_init::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    @timeit to "update" @jlaunch tend_apply_kernel mini input(state_init, tend, hy_dens_cell, dt) output(state_out, tend) hip(threads=THREADS, enable_if=USE_HIP) cuda(threads=THREADS) cuda(threads=THREADS, enable_if=USE_CUDA)
...
function set_halo_values_x!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    if NRANKS == 1
        ...
        #THREADS = ((1, NZ, NUM_VARS), 1)
        @jlaunch halo_1rank_kernel mini input(state) output(state) hip(threads=THREADS, enable_if=USE_HIP) cuda(threads=THREADS, enable_if=USE_CUDA)
        return
    end
    ...
    @jlaunch halo_sendbuf_kernel mini input(state) output(sendbuf_l, sendbuf_r
                    ) hip(threads=THREADS, enable_if=USE_HIP) cuda(threads=THREADS, enable_if=USE_CUDA)
    ...
    if (DATA_SPEC == DATA_SPEC_INJECTION)
       if (MYRANK == 0)
            ...
            #THREADS = ((1, NZ, 1), 1)
            @jlaunch halo_inject_kernel mini input(state, hy_dens_cell,
                        hy_dens_theta_cell) output(state) hip(threads=THREADS, enable_if=USE_HIP) cuda(threads=THREADS, enable_if=USE_CUDA, enable_if=USE_CUDA)
...
function compute_tendencies_x!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    #THREADS = ((NX+1, NZ), 1)
    @jlaunch tend_x_kernel mini input(state, dt,hy_dens_cell, hy_dens_theta_cell
                    ) output(flux, tend) hip(threads=THREADS, enable_if=USE_HIP)  cuda(threads=THREADS, enable_if=USE_CUDA)

    blk_x = div(NX+THREADS_PER_BLOCK-1, THREADS_PER_BLOCK)
    THREADS = ((blk_x, NZ, NUM_VARS),(THREADS_PER_BLOCK, 1, 1))
    #THREADS = ((NX, NZ, NUM_VARS),1)
    @jlaunch tend_x_calc_kernel mini input(flux) output(tend) hip(threads=THREADS, enable_if=USE_HIP)  cuda(threads=THREADS, enable_if=USE_CUDA)
...
function set_halo_values_z!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    @jlaunch halo_z_kernel mini input(state, hy_dens_cell) output(state) hip(threads=THREADS, enable_if=USE_HIP) cuda(threads=THREADS, enable_if=USE_CUDA)
...
function compute_tendencies_z!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    #THREADS = ((NX, NZ+1),1)
    @jlaunch tend_z_kernel mini input(state, dt,hy_dens_int, hy_dens_theta_int,
                    hy_pressure_int) output(flux, tend) hip(threads=THREADS, enable_if=USE_HIP) cuda(threads=THREADS, enable_if=USE_CUDA)

    blk_x = div(NX+THREADS_PER_BLOCK-1, THREADS_PER_BLOCK)
    THREADS = ((blk_x, NZ, NUM_VARS),(THREADS_PER_BLOCK, 1, 1))
    #THREADS = ((NX, NZ, NUM_VARS),1)
    @jlaunch tend_z_calc_kernel mini input(state, flux) output(tend) hip(threads=THREADS, enable_if=USE_HIP) cuda(threads=THREADS, enable_if=USE_CUDA)
