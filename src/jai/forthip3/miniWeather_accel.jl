...
function main(args::Vector{String})
    ...
    @jkernel PATH_TEND_APPLY1_KERNEL tend_apply1_kernel mini framework(hip=COMPILE_HIP, fortran=COMPILE_FORTRAN) 
    @jkernel PATH_TEND_APPLY2_KERNEL tend_apply2_kernel mini framework(hip=COMPILE_HIP, fortran=COMPILE_FORTRAN) 
...
function compute_tendencies_z!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    #@jenterdata updateto(state)
    THREADS = ((NX, NZ+1),1)
    @jlaunch tend_z_kernel mini input(state, dt,hy_dens_int, hy_dens_theta_int, hy_pressure_int) output(flux, tend) hip(threads=THREADS)
#    @jexitdata mini updatefrom(flux)
