...
const COMPILE_FOPENACC_CRAY = "pgfortran -shared -fPIC -acc"
const COMPILE_FORTRAN = "pgfortran -fPIC -shared"

const PATH_REDUCTION_KERNEL = joinpath(@__DIR__, "reduction.knl") 
const PATH_TEND_APPLY_KERNEL = joinpath(@__DIR__, "tend_apply.knl") 
const PATH_TEND_APPLY_KERNEL_SAME = joinpath(@__DIR__, "tend_apply_same.knl") 
const PATH_TEND_APPLY_KERNEL_DIFF = joinpath(@__DIR__, "tend_apply_diff.knl") 
...
function main(args::Vector{String})
    ...
    @jkernel mini tend_apply_kernel_same PATH_TEND_APPLY_KERNEL_SAME framework(cuda="nvcc -shared --linker-options=\"-fPIC\" -g")
    #@jkernel mini tend_apply_kernel_same PATH_TEND_APPLY_KERNEL_SAME
    @jkernel mini tend_apply_kernel_diff PATH_TEND_APPLY_KERNEL_DIFF framework(cuda="nvcc -shared --linker-options=\"-fPIC\" -g")
    #@jkernel mini tend_apply_kernel_diff PATH_TEND_APPLY_KERNEL_DIFF
    ...
    ### TEST output(state,etime,nt,hy_dens_cell,hy_dens_theta_cell)
...
function init!()
    ...
    _statetmp   = zeros(Float64, NX+2*HS, NZ+2*HS, NUM_VARS) 
...
#    @jlaunch mini tend_apply_kernel input(state_init, tend, hy_dens_cell, dt) output(tend, state_out)

    if state_init !== state_out
        state = state_init
        statetmp = state_out
        @jlaunch mini tend_apply_kernel_diff input(state, tend, hy_dens_cell, dt) output(tend, statetmp) cuda("chevron"=>((NUM_VARS, NZ, NX), 1))
        #@jlaunch mini tend_apply_kernel_diff input(state, tend, hy_dens_cell, dt) output(tend, statetmp)
    else
        state = state_init
        @jlaunch mini tend_apply_kernel_same input(state, tend, hy_dens_cell, dt) output(tend, state) cuda("chevron"=>((NUM_VARS, NZ, NX), 1))
        #@jlaunch mini tend_apply_kernel_same input(state, tend, hy_dens_cell, dt) output(tend, state) cuda("chevron"=>(1, 1))
        #@jlaunch mini tend_apply_kernel_same input(state, tend, hy_dens_cell, dt) output(tend, state)
    end
