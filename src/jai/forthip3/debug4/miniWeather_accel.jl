...
function semi_discrete_step!(state_init::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    #@jenterdata mini updateto(state_forcing, state_init, state_out, tend)
...
function set_halo_values_x!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    if NRANKS == 1
        ...
        @jexitdata updatefrom(state)
        println("-- halo_1rank_kernel", sum(state))
        return
    end
...
function compute_tendencies_x!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    println("TTTTTTTTTTTTTT sum(state) = ", sum(state))
    #THREADS = ((NZ, NX+1),1)
    THREADS = (1, (NZ, NX+1))
...
function set_halo_values_z!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    @jexitdata updatefrom(state)
    println("-- halo_z_kernel", sum(state))
...
function compute_tendencies_z!(state::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    @jexitdata updatefrom(state, flux)
    println("-- tend_z_kernel", sum(flux))

    THREADS = ((NUM_VARS, NZ, NX),1)
    @jlaunch tend_z_calc_kernel mini input(state, flux) output(tend) hip(threads=THREADS)
    @jexitdata updatefrom(tend)
    println("-- tend_z_calc_kernel", sum(tend))
