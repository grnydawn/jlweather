...
function main(args::Vector{String})
    ...
    @jkernel mini tend_apply_kernel_same PATH_TEND_APPLY_KERNEL_SAME framework(hip="hipcc -shared -fPIC -lamdhip64 -g")
    #@jkernel mini tend_apply_kernel_same PATH_TEND_APPLY_KERNEL_SAME
    @jkernel mini tend_apply_kernel_diff PATH_TEND_APPLY_KERNEL_DIFF framework(hip="hipcc -shared -fPIC -lamdhip64 -g")
...
function semi_discrete_step!(state_init::OffsetArray{Float64, 3, Array{Float64, 3}},
    ...
    if state_init !== state_out
        state = state_init
        statetmp = state_out
        @jlaunch mini tend_apply_kernel_diff input(state, tend, hy_dens_cell, dt) output(tend, statetmp) hip("chevron"=>((NUM_VARS, NZ, NX), 1))
        #@jlaunch mini tend_apply_kernel_diff input(state, tend, hy_dens_cell, dt) output(tend, statetmp)
    else
        state = state_init
        @jlaunch mini tend_apply_kernel_same input(state, tend, hy_dens_cell, dt) output(tend, state) hip("chevron"=>((NUM_VARS, NZ, NX), 1))
        #@jlaunch mini tend_apply_kernel_same input(state, tend, hy_dens_cell, dt) output(tend, state) hip("chevron"=>(1, 1))
